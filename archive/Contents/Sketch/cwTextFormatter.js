...
function cwTextFormatter(context) {
    ...
    function updateTextStyles() {
        ...
        if(fontIndex === 0) {
            layerName = 'o ' + typeSizesLayerLabelList[sizeIndex];
        } else {
            layerName = 'p ' + typeSizesLayerLabelList[sizeIndex];
        }
        if(fontWeightsListLayerSuffix[fontIndex][sizeIndex][weightIndex] !== '') {
            layerName += ' ' + fontWeightsListLayerSuffix[fontIndex][sizeIndex][weightIndex];
        }
        for(var i = 0; i < selection.count(); i++) {
            if(selection[i].class() == textObjectClass) {
                textLayer = selection[i];
                textFont = NSFont.fontWithName_size_(fontNameAndWeight, typeSizesList[sizeIndex]);
                var newLayerName = textLayer.name().replace(/[ ]?\[[a-z\- ]+\]/i, '');
                textLayer.setName(newLayerName + ' [' + layerName + ']');
